...
class NaverBlogCrawler:
    ...
    def run(self, keywords, days=180, output_file="blog_counts.csv"):
        results = []
...
if __name__ == "__main__":
    # Sample keywords
    target_keywords = ["두바이쫀득쿠키", "두쫀쿠"]
    
    crawler = NaverBlogCrawler()
    # CSV 경로 설정 (실행 위치에 따라 조정 필요)
    csv_path = "blog_counts.csv"
    crawler.run(target_keywords, days=180, output_file=csv_path)
