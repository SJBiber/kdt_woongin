...
class YouTubeTrendCrawler:
    ...
    def get_historical_data(self, keyword, start_date=None, end_date=None):
        ...
        #    영상 통계는 실시간으로 변하므로, 어제까지의 데이터를 매일 새로 업데이트합니다.
        if end_date is None:
            end_date = datetime.now(timezone.utc) - timedelta(days=1)
            
        # 2. 시작일 설정: 날짜 설정이 없으면 '2025-07-17' 고정 날짜(프로젝트 시작 시점)를 사용합니다.
        #    이 날짜부터 어제까지의 모든 데이터를 다시 훑어서 최신 조회수 등을 반영합니다.
        if start_date is None:
            start_date = datetime(2025, 7, 17, tzinfo=timezone.utc)
        
        print(f"[*] 분석 시작 범위: {start_date.strftime('%Y-%m-%d')} ~ {end_date.strftime('%Y-%m-%d')} (전체 통계 갱신)")
