...
*   **수집 출처**: 네이버 블로그 검색 API, 네이버 데이터 랩, 구글 트렌드, 각 쇼핑몰 원자재 가격 정보, 유튜브 데이터 API (v3), 국제 원자재 시세 보도자료.
*   **수집 주기**: 프로젝트 기간 내 배치(Batch) 수집 및 시계열 분석.
*   **데이터 볼륨**: 네이버 블로그 두쫀쿠 및 **과거 유행 키워드(탕후루, 요아정 등)** 포스트 업로드량, 유튜브 키워드 업로드량 및 일별 조회수, 댓글 수, 좋아요 수, 구글 트렌드 및 네이버 키워드 검색량.
...
- [ ] Supabase 테이블 스키마 설계 및 생성 (Index 적용)
